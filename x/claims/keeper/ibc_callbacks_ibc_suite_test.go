...
func (suite *IBCTestingSuite) TestOnAcknowledgementPacketIBC() {
	sender := "hhub1sv9m0g7ycejwr3s369km58h5qe7xj77hmzdks8"   //nolint:goconst
	receiver := "hhub1hf0468jjpe6m6vx38s97z2qqe8ldu0nj67z0pr" //nolint:goconst
	...
	testCases := []struct {
		...
		},
		{
			...
			func(claimableAmount int64) {
				...
				coins := sdk.NewCoins(sdk.NewCoin("ahhub", amt))
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			transfer := transfertypes.NewFungibleTokenPacketData("ahhub", "100", sender, receiver, "")
			...
			coin := suite.chainA.App.(*app.Evmos).BankKeeper.GetBalance(suite.chainA.GetContext(), senderAddr, "ahhub")
			suite.Require().Equal(sdk.NewCoin("ahhub", sdk.NewInt(tc.expectedBalance)).String(), coin.String())
...
func (suite *IBCTestingSuite) TestOnRecvPacketIBC() {
	sender := "hhub1hf0468jjpe6m6vx38s97z2qqe8ldu0nj67z0pr"
	receiver := "hhub1sv9m0g7ycejwr3s369km58h5qe7xj77hmzdks8"
	...
	testCases := []struct {
		...
		},
		{
			...
			func(claimableAmount int64) {
				amt := sdk.NewInt(claimableAmount)
				coins := sdk.NewCoins(sdk.NewCoin("ahhub", sdk.NewInt(claimableAmount/4)))
		...
		},
		{
			...
			func(claimableAmount int64) {
				amt := sdk.NewInt(claimableAmount)
				coins := sdk.NewCoins(sdk.NewCoin("ahhub", amt.Add(amt.QuoRaw(2))))
		...
		},
		{
			...
			func(claimableAmount int64) {
				amt := sdk.NewInt(claimableAmount)
				coins := sdk.NewCoins(sdk.NewCoin("ahhub", amt.QuoRaw(2)))
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			transfer := transfertypes.NewFungibleTokenPacketData("ahhub", triggerAmt, sender, receiver, "")
			bz := transfertypes.ModuleCdc.MustMarshalJSON(&transfer)
			packet := channeltypes.NewPacket(bz, 1, path.EndpointA.ChannelConfig.PortID, path.EndpointA.ChannelID, path.EndpointB.ChannelConfig.PortID, path.EndpointB.ChannelID, timeoutHeight, 0)
			...
			coin := suite.chainB.App.(*app.Evmos).BankKeeper.GetBalance(suite.chainB.GetContext(), receiverAddr, "ahhub")
			suite.Require().Equal(coin.String(), sdk.NewCoin("ahhub", sdk.NewInt(tc.expectedBalance)).String())
