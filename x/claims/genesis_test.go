...
var (
	acc1 = sdk.MustAccAddressFromBech32("hhub1qxx0fdsmruzuar2fay88lfw6sce6emamyumprap8")
	acc2 = sdk.MustAccAddressFromBech32("hhub1nsrs4t7dngkdltehkm3p6n8dp22sz3mculhzns")
)
...
func (suite *GenesisTestSuite) TestClaimInitGenesis() {
	testCases := []struct {
		...
		},
		{
			...
			func() {
				coins := sdk.NewCoins(sdk.NewCoin("ahhub", sdk.NewInt(2_800)))
		...
		},
		{
			...
			func() {
				coins := sdk.NewCoins(sdk.NewCoin("ahhub", sdk.NewInt(400)))
				err := testutil.FundModuleAccount(suite.ctx, suite.app.BankKeeper, types.ModuleName, coins)
				suite.Require().NoError(err)
			},
			false,
